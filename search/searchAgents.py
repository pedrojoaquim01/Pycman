...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (self.startingPosition,self.estadoInic)
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            children.append((self.getNextState(state, action),action,self.getActionCost(state,action,self.getNextState(state, action))))
    ...
    def getNextState(self, state, action):
        ...
        if not self.walls[nextx][nexty]:
                ...
                if (nextx,nexty) in self.corners:
                    canto[self.corners.index((nextx,nexty))] = 1

                return ((nextx, nexty), tuple(canto))
        util.raiseNotDefined()
    ...
    if problem.isGoalState(state):
        ...
        for index,item in enumerate(state[1]):
            if item == 0: 
                ...
                xy1 = state[0]
                xy2 = corners[index]
                distanciaDosCantos.append(abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]))
        return max(distanciaDosCantos)
...
def foodHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        distanciaDosCantos = [] 
        index = 0
        while index < len(listaFood):
                xy1 = position
                xy2 = listaFood[index]
                #Usando a função mazeDistance em vez do manhatam normal que usei antes
                distanciaDosCantos.append(mazeDistance(xy1,xy2,problem.startingGameState))
                index = index + 1
        return max(distanciaDosCantos)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        listaComida = self.food.asList()

        # State reach food #
        if (x,y) in listaComida:
            return True
        else:
            return False
        util.raiseNotDefined()
